...
if __name__ == "__main__":
    user_desired_players = get_desired_players()
    roster = pd.DataFrame(index=['foo', 'bar'], data=np.array([['john smith', 'QB', 0.0], ['jon steve', 'WR', 1.0]]),
                          columns=['full_name', 'position', 'points'])
    roster.loc['np'] = ['another','BENCH',5.0]
    print(roster)
